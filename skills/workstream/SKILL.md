...
name: workstream
description: >
  Coordinate long-running, multi-task work with durable context and bounded execution.
  Use when one lead owns ongoing planning, delegation, and integration.
  Not for a self-contained task without persistent coordination.
---
...
默认 `capture + wip_limit: 4`：新需求先进入 backlog，不因有容量就自动执行。明确的一次性排期、readiness、WIP 计数与依赖规则见 [references/task-lifecycle-and-scheduling.md](references/task-lifecycle-and-scheduling.md)；只有真正排期时才展开。

运行时 context 使用 `schema_version: 2`：`tasks/*.md` 是唯一的 open Task board，`receipts/<task-id>/<attempt-id>.md` 保存 `AT-*` attempt evidence，`reviews/<review-id>.md` 是 `RV-*` owner attention queue。`state.md` frontmatter 保存执行控制，正文才是可重建 projection；不要重建时丢失暂停状态。Task acceptance 使用稳定的 `AC-*` ID，Task lifecycle 与 Lead verification 分开。

## Run admission

Workstream 是长期目标，Run 是一次有界执行；长期目标未完成不要求模型持续醒着。涉及自主 Loop、预算、自动唤醒或暂停恢复时，先读取 [references/run-control.md](references/run-control.md)，在加载完整历史、刷新外部状态或派发前检查 `state.md` 的 Run gate。

- 自动续跑不是新的用户授权。已暂停且恢复条件未满足时，不重新规划、查额度或重复发暂停提示。
- `capture` 不是暂停开关；预算保护不修改原 execution mode。停止业务操作也不等于停止外层唤醒。
- 无人值守启动前，确认当前宿主有可调用、可验证的停机路径。无法兑现硬额度保留时说明限制，不启动无界 Goal 再等用户回来停机。
- 请求停止后必须核对宿主已停止；不得通过假报业务完成或滥用 blocked 关闭 Goal。
...
3. 首次进入或需要迁移/归档时，读取 [references/context-lifecycle.md](references/context-lifecycle.md)；真正排期或处理 Task 状态时读取 [references/task-lifecycle-and-scheduling.md](references/task-lifecycle-and-scheduling.md)。已加载且未变化的协议不重复读取。
4. 加载最小充分 context，并从代码、仓库、任务系统或其它权威来源刷新即将影响决策的当前事实。Context 是协调缓存，不是当前事实的替代品。
5. 将新需求转化为 outcome、scope、带 `AC-*` 的 acceptance、typed dependencies/blockers、readiness 与待验证假设。默认只写入 backlog；若用户明确点名 Task 执行/排期，记录 one-shot schedule request 并按 runnable predicate 判断；只有缺失信息会实质改变目标、权限或外部影响时才询问用户。

## Lead loop

先区分新需求、明确执行/恢复、worker 回报、自动唤醒和只读状态查询。只有新需求需要重新塑形；自动唤醒沿用已有 scope，只处理新证据或可执行增量。Run gate 允许后再进入闭环：
...
7. **Report**：有实质变化、需要 owner 决策或用户主动查询时输出 closeout；平时只报告增量，里程碑与用户完整查询时展开六段看板。没有变化不重复输出。预算停机只做一次必要交接，不制造汇报循环。
...
Owner review 是非阻塞的 attention queue，不是默认审批 gate。只有 Task 的 typed blocker 引用 active `decide` review 时才阻塞该 Task。需要 closeout 时，展示新 queued 内容；已有 presented 项不反复展开。具体 intent、状态、展示与压缩规则见 [references/delegation-and-coordination.md](references/delegation-and-coordination.md)。
...
Work item 或 Run 结束不等于 workstream 完成。保留 active context 和未执行 backlog，直到用户明确表示整个 workstream 已完成、取消或需要清理；保留记录不意味着持续消耗模型额度。
